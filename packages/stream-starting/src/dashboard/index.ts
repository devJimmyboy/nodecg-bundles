...
const eVideoPreview = $(
  `<video id="video" hidden loop width="100%"><source id="webm" type="video/webm"/></video>`
);
...
const dConfigUpdate = debounce((e: JQuery.ChangeEvent<HTMLInputElement>) => {
  switch (e.target.id) {
    case "length":
      states.value[currentState.value].length = $("#length")
        .val()
        .valueOf() as number;
      break;
    case "custom-message":
      states.value[currentState.value].loadingText =
        eCustomMessage.val() as string;
      break;
  }
}, 250);
...
$(() => {
  ...
  NodeCG.waitForReplicants(tVideos, videos, states, currentState).then(() => {
    ...
    eVideoSelect.on("change", (e) =>
      loadVideo(videos.value.find((v) => v.name === $(e.target).val()).url)
    );
    $("#saveVideoButton").on("click", (e) => {
      e.preventDefault();
      console.log(
        "bg video changed to %c%s",
        "color: cyan;",
        eVideoSelect.val()
      );
  ...
  NodeCG.waitForReplicants(progress, states, currentState, videos).then(() => {
    ...
    progress.on("change", function (newVal, oldVal) {
      ...
      eTimeLeftValue.text(
        `${timeLeft?.min}m ${timeLeft?.sec}s ${timeLeft?.ms}ms`
      );
    });
  ...
  eStateButtons.on("click", function (e) {
    ...
    if (checked.is(":checked")) {
      nodecg.sendMessage(
        "changeState",
        checked.siblings("span").text().toLowerCase()
      );
    }
  });
});
