...
export default class Chat implements TwitchModule {
  ...
  constructor(twitch: Twitch) {
    ...
    this.reps.projectResponses = this.nodecg.Replicant('projectResponses', {
      defaultValue: [{ name: 'none', active: true, response: "Jimmy isn't working on anything at the moment :)" }],
      persistent: true,
    })
  }
  ...
  public async registerListeners() {
    ...
    this.client.onMessage((chan, user, message, _msg) => {
      ...
      if (this.isMod(_msg) && (message.startsWith('!shoutout') || message.startsWith('!so'))) {
        const userToShoutOut = message.split(' ')[1]
        if (userToShoutOut) {
          this.getAClip(userToShoutOut)
            .then((args) => {
              if (!args) {
                this.client.say(this.channel, `@${user} No clip found for ${userToShoutOut} (or they don't exist)`)
                return
              }
              const { clip, userName } = args
              this.twitch.sendAlert('shoutout', `Shoutout to (${userName})! Here's an (epic) (clip) from them this past week.`, '', clip)
            })
            .catch(() => this.client.say(this.channel, 'No clip found for that user'))
        }
      }
    })
    ...
    this.client.onSub((channel, user, subInfo) => {
      ...
      if (streak && months > streak) msg += `They've got a (${streak}) month streak out of (${months})!`
      else msg += `It's their (${months % 10 < 4 ? months + (months % 10 == 2 ? (months % 10 == 1 ? `st` : 'nd') : 'rd') : `${months}th`}) month!`
      this.twitch.sendAlert(type, msg, message)
    })
    ...
    this.client.onRaid(async (channel, user, raidInfo) => {
      ...
      if (!AClip) {
        this.twitch.sendAlert('raid', `(${displayName}) is raiding with (${viewerCount}) gamers!`, 'Nice!')
      } else {
        const { clip } = AClip

        this.twitch.sendAlert('shoutout', `(${displayName}) is raiding with (${viewerCount}) gamers!`, 'Nice', clip)
      }
    })
    this.client.onHosted(async (channel, byChannel, auto, viewers) => {
      ...
      if (!AClip) {
        this.twitch.sendAlert('host', `(${byChannel}) just hosted me with (${viewers}) gamers!`, 'Nice!')
      } else {
        const { clip, userName } = AClip

        this.twitch.sendAlert('shoutout', `(${userName}) just hosted me with (${viewers}) gamers!`, 'Nice!', clip)
      }
    })
  }
  // Utility Commands
  ...
  async getAClip(user: HelixUser | string) {
    ...
    let clips = (await this.api.clips.getClipsForBroadcaster(user.id, { startDate: moment().subtract(7, 'd').toISOString() })).data
    clips.sort((a, b) => b.views - a.views)
    return { clip: this.getClipURL(clips[0].thumbnailUrl), userName: user.displayName }
  }
