import {
  TwitchAddonsClient,
  EmoteCollection,
} from "nodecg-io-twitch-addons/extension"
...
module.exports = async function (nodecg: NodeCGServer, twitch: Twitch, twitchAddonsProvider: ServiceProvider<TwitchAddonsClient>) {
  ...
  twitchAddonsProvider?.onAvailable(async (client) => {
    ...
    if (emotes === undefined)
      emotes = await client.getEmoteCollection("devjimmyboy", { includeGlobal: true, include7tv: true }).then((res) => emotesRep.value = res)
  ...
  nodecg.listenFor("getBadges", async (user: string, ack) => {
    if (!twitch.ready) {
      if (ack && !ack.handled)
        ack(new Error("twitch isn't ready"));
      return
    }
