...
function makeTheme(themeColors: { [key: string]: string }) {
  return Object.entries(themeColors).map(([token, colorPath]) => ({
    token,
    foreground: getColor(colorPath),
  }))
}
export default function addThemes(monaco: Monaco) {
  monaco.editor.defineTheme("tw-light", {
    base: "vs",
    inherit: true,
    rules: [
      { token: "", foreground: getColor("gray.800") },
      ...makeTheme({
        comment: "gray.400",
        string: "indigo.600",
        number: "gray.800",
  ...
  monaco.editor.defineTheme("tw-dark", {
    ...
    inherit: true,
    rules: [
      { token: "", foreground: getColor("slate.50") },
      ...makeTheme({
        comment: "slate.400",
        string: "sky.300",
        number: "slate.50",
