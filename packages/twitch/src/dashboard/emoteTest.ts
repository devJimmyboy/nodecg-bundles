    ...
    debounce(async function (e) {
      ...
      if (!chatMsgMode) {
        nodecg.sendMessage("getEmote", emote).then(function (data) {
          console.log(data);
          if (data) emoteImg.attr("src", data);
        });
      } else {
        let options = {
          message: emote as string,
          options: {
            classListEmoteImg: ["emote"],
            classListEmoteSpan: [],
            classListWords: ["msg-words"],
          },
        };
        nodecg.sendMessage("parseEmotes", options).then(async function (data) {
          ...
          if (data)
            $(`<div class="msg-wrapper"><div class="msg"></div></div>`)
              .appendTo(emoteOut)
              .prepend(await getUserNameDiv())
              .children(".msg")
              .html(data);
        });
      }
...
let emotes = nodecg.Replicant<any | undefined>("emotes");

emotes.on("change", (newVal, oldVal) => {
  if (newVal) updateList(newVal);
});

function updateList(emotes: any) {
  emoteList.empty();
  for (let prov in emotes) {
    let emoteSet = emotes[prov];
    let emoteSetDiv = $(
      `<div class="emote-set collapse-close"><div class="collapse-title">${prov}</div></div>`
    )
      .appendTo(emoteList)
      .on("click", (e) => {
    ...
    let emoteSetList = $(`<div class="collapse-content"></div>`).appendTo(
      emoteSetDiv
    );
    if (prov.includes("ffz"))
      emoteSet.forEach((emoteSetffz: any, i) => {
        emoteSetffz.emoticons.forEach((emote, i) => {
          let emoteName = emote.name;
          let emoteDiv = $(
            `<div class="emote-item">${emoteName}</div>`
          ).appendTo(emoteSetList);
          emoteDiv.on("click", function () {
            emoteInput
              .val(
                chatMsgMode
                  ? emoteInput.val().toString().trim() + " " + emoteName
                  : emoteName
              )
              .trigger("input");
          });
      ...
      emoteSet.forEach((emote, i) => {
        ...
        let emoteDiv = $(`<div class="emote-item">${emoteName}</div>`).appendTo(
          emoteSetList
        );
