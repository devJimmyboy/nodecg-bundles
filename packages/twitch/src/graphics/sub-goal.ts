...
import defaultCssText from './sub-goal.css?url'
  ...
  .then(() => {
    ...
    nodecg.listenFor('refresh', (msg) => {
      ...
      if (subs >= currentGoal.goal) {
        ...
        if (celebrate.playing) celebrate.end()
      }
    })
    if (!debug) currentSubs.on('change', onSubChange)
  })
...
function initDebug() {
  ...
  const debugSubs = $<HTMLInputElement>("<input type='range' step='1' min='0' max='100'>")
    .addClass(['range'])
    .appendTo(debug)
    .on('change', (e) => onSubChange(parseInt(e.target.value)))
