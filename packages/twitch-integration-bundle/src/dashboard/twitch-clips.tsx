...
function App() {
  ...
  const [videoReplicant] = useReplicant<TwitchClip[], TwitchClip[]>("twitchClips", [])
  ...
  const clipSorter = (a: TwitchClip, b: TwitchClip) =>
  ...
  return (
    <>
      ...
      <div className={"list-scroller"}>
        {showSelected
          ? Object.values(selectedClipsReplicant)
              .sort(clipSorter)
              .map((video) => (
                <SelectedVideoInfo
                  key={video.id}
                  video={video}
                  isSelected={false}
                  showVideo={showVideo(video)}
                  toggleSelect={toggleSelect(video)}
                />
              ))
    ...
    <div key={video.id} className={"list-item"}>
      <div className={"list-text"}>
        <div className={"list-title"}>
          {video.creator_name} - {video.title}
        </div>
      ...
      <div className={"list-buttons"}>
        ...
        <br />
        <button onClick={toggleSelect}>{isSelected ? "Select" : "Unselect"}</button>
      </div>
    </div>
  )
}
