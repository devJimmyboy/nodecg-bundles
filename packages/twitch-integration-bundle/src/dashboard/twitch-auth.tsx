...
export function App() {
  ...
  const [twitchCredentials, setTwitchCredentials] = useReplicant<TwitchCredentials | any, TwitchCredentials | any>(
    "twitchCredentials",
    {
      clientId: "",
      clientSecret: "",
  ...
  if (twitchCredentials.isConnected)
    return (
      <>
        ...
        <button className="twitch" onClick={handleLogout}>
          Logout
        </button>
      </>
    )
  ...
  return (
    <>
      <a className="twitch" href="https://dev.twitch.tv/console/apps" target="_blank">
        Twitch Dev Console
      </a>
      ...
      <input
        ...
        value={twitchCredentials.clientSecret}
        name="clientSecret"
        type="password"
      />
      <button className="twitch" onClick={handleSignIn} disabled={isSignInDisabled}>
        Sign In
      </button>
    </>
  )
