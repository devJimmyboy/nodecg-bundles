...
import { useReplicant } from "../lib/use-nodecg"

const isRedemptionEvent = (event: TwitchEvent): event is TwitchRedemptionEvent =>
  (event as TwitchRedemptionEvent).data.timestamp !== undefined
const isSubscriptionEvent = (event: TwitchEvent): event is TwitchSubscriptionEvent =>
  (event as TwitchSubscriptionEvent).data.sub_message !== undefined
const isBitsEvent = (event: TwitchEvent): event is TwitchBitsEvent =>
  (event as TwitchBitsEvent).data.bits_used !== undefined
const isBitsBadgeEvent = (event: TwitchEvent): event is TwitchBitsBadgeEvent =>
  (event as TwitchBitsBadgeEvent).data.badge_tier !== undefined
    ...
    <div className="event-section">
      <div className="event-header">
        ...
        <div className="event-header-buttons">
          <div onClick={handleResendEvent}>
            <RefreshIcon />
          </div>
          <div onClick={handleShowChildren}>
            <DownChevronIcon />
          </div>
        </div>
      </div>
      {showChildren && <div className="event-children">{children}</div>}
    </div>
  )
}
...
function PubSubSubscriptionEvent({ event }: PubSubSubscriptionEventProps) {
  ...
  return (
    <EventSection
      ...
      {event.data.context === "sub" ||
        (event.data.context === "resub" && <DataSection name="User" value={event.data.display_name} />)}
...
function PubSubBitEvent({ event }: PubSubBitEventProps) {
  return (
    <EventSection
      ...
      event={event}>
      <DataSection name="User" value={event.data.is_anonymous ? <i>Anonymous</i> : event.data.user_name} />
...
export function App() {
  const [twitchEvents] = useReplicant<TwitchEvent[], TwitchEvent[]>("twitchEvents", [])
  ...
  return (
    <>
      ...
      <div className="action-buttons">
        ...
        <span>
          Showing first {max} of {twitchEvents.length}
        </span>
        <div className="button">
          <button className="less" onClick={handleShowLess}>
            Less
          </button>
