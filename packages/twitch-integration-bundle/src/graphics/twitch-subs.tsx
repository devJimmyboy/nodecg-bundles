...
function App() {
  ...
  const [availableVideos] = useReplicant<{ [id: string]: TwitchClip }, { [id: string]: TwitchClip }>(
    "twitchSelectedClips",
    {}
  )
  ...
  const videoOnEnded = () => {
    setArrayIndex((p) => {
      p = p + 1
      if (p >= videoArray.length) return 0
      return p
    })
