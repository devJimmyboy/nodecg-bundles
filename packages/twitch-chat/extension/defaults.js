...
module.exports = {
    Chat = {
        info: {
            channel: null,
            animate: "animate" in $.QueryString
                ? $.QueryString.animate.toLowerCase() === "true"
                : false,
            showBots: "bots" in $.QueryString
                ? $.QueryString.bots.toLowerCase() === "true"
                : false,
            hideCommands: "hide_commands" in $.QueryString
                ? $.QueryString.hide_commands.toLowerCase() === "true"
                : false,
            hideBadges: "hide_badges" in $.QueryString
            ...
            smallCaps: "small_caps" in $.QueryString
            ...
            cheers: {},
            lines: [],
            blockedUsers: "block" in $.QueryString
            ...
            [
                ...
                $.getJSON("https://api.betterttv.net/3/cached/" + endpoint).done(function (res) {
                    ...
                    res.forEach((emote) => {
                        Chat.info.emotes[emote.code] = {
                            id: emote.id,
                            image: "https://cdn.betterttv.net/emote/" +
                                emote.id +
                                "/3x",
                            zeroWidth: [
                            ...
                            ].includes(emote.id), // "5e76d338d6581c3724c0f0b2" => cvHazmat, "5e76d399d6581c3724c0f0b8" => cvMask, "567b5b520e984428652809b6" => SoSnowy, "5849c9a4f52be01a7ee5f79d" => IceCold
                        };
                    });
                });
            });
        ...
        load: function (callback) {
            TwitchAPI("https://api.twitch.tv/v5/users?login=" + Chat.info.channel).done(function (res) {
                ...
                TwitchAPI("https://badges.twitch.tv/v1/badges/global/display").done(function (global) {
                    Object.entries(global.badge_sets).forEach((badge) => {
                        Object.entries(badge[1].versions).forEach((v) => {
                            Chat.info.badges[badge[0] + ":" + v[0]] =
                                v[1].image_url_4x;
                        });
                    });
                    TwitchAPI("https://badges.twitch.tv/v1/badges/channels/" +
                        encodeURIComponent(Chat.info.channelID) +
                        "/display").done(function (channel) {
                        ...
                        $.getJSON("https://api.frankerfacez.com/v1/_room/id/" +
                            encodeURIComponent(Chat.info.channelID)).done(function (res) {
                ...
                // Load cheers images
                TwitchAPI("https://api.twitch.tv/v5/bits/actions?channel_id=" +
                    Chat.info.channelId).done(function (res) {
                ...
                Chat.info.ffzapBadges.forEach((user) => {
                    if (user.id.toString() === userId) {
                        ...
                        var userBadge = {
                            ...
                            url: "https://api.ffzap.com/v1/user/badge/" +
                                userId +
                                "/3",
                            color: color,
                        };
                        ...
                        switch (message.command) {
                            ...
                            case "PRIVMSG":
                                if (message.params[0] !== "#" + channel ||
                                    !message.params[1])
                                    return;
                                var nick = message.prefix
                                    .split("@")[0]
                                    .split("!")[0];
                                if (message.params[1].toLowerCase() ===
                                    "!refreshoverlay" &&
