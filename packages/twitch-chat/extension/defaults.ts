module.exports = {
	...
	info: {
		...
		stroke:
			"stroke" in $.QueryString ? parseInt($.QueryString.stroke) : false,
		shadow:
			"shadow" in $.QueryString ? parseInt($.QueryString.shadow) : false,
		smallCaps:
	...
	loadEmotes: function (channelID) {
		...
		[
			...
			$.getJSON("https://api.7tv.app/v2/" + endpoint).done(function (
				res
			) {
				res.forEach((emote) => {
	...
	load: function (callback) {
		TwitchAPI(
			"https://api.twitch.tv/v5/users?login=" + Chat.info.channel
		).done(function (res) {
				...
				Chat.info.ffzapBadges.forEach((user) => {
					if (user.id.toString() === userId) {
						...
						var userBadge = {
							...
							url:
								"https://api.ffzap.com/v1/user/badge/" +
								userId +
								"/3",
							color: color,
						};
			...
			if (Chat.info.hideBadges) {
				if (typeof info.badges === "string") {
					info.badges.split(",").forEach((badge) => {
						...
						$badge.attr(
							"src",
							Chat.info.badges[badge[0] + ":" + badge[1]]
						);
			...
			socket.onmessage = function (data) {
				data.data.split("\r\n").forEach((line) => {
					...
					switch (message.command) {
						...
						case "JOIN":
							console.log(
								"jChat: Joined channel #" + Chat.info.channel
							);
							return;
						case "CLEARMSG":
							if (message.tags)
