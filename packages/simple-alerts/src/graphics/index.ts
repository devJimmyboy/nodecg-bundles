...
function filterInit() {
  var badWords = nodecg.Replicant<string[]>("badWords", {
...
function animateText(word: string) {
  ...
  span.setAttribute(
    "style",
    "color: " + getKeywordColour(activeAlert.value) + " ; position: relative;"
  );
  ...
  activateAlert.on("change", async (value) => {
    ...
    if (alertQueue.value.length > 0) {
      ...
      videoInd = isShoutout
        ? -1
        : Math.floor(
            Math.random() * alerts.value[activeAlert.value]?.media.length
          );
      console.log(
        "video index is ",
        videoInd,
        "num of medias is ",
        activeAlert.value !== -1
          ? alerts.value[activeAlert.value]?.media.length
          : "Shoutout"
      );
      ...
      // Handle media
      if (
        (isShoutout && typeof activateAlert.value.event === "string") ||
        getMediaExt(activeAlert.value) === ".webm"
      ) {
        console.log("Alert is Video");
        var video = $(
          `<video class="transition-all"/>`
        ) as JQuery<HTMLVideoElement>;
        var src = $<HTMLSourceElement>(
          `<source id='webm' type='video/${isShoutout ? "mp4" : "webm"}'/>`
        ).appendTo(video);
        video
          .attr("id", "webm-video")
      ...
      if (video)
        video.on("ended", async (e) => {
          gsap.to(video, {
            ...
            scale: 0.25,
            duration: 1.25,
            ease: "elastic.in(1.4,0.8)",
          });
          if (activateAlert.value.attachedMsg)
            await playSound(activeAlert.value, activateAlert.value.attachedMsg);
          gsap.to("#alert-user-message, #alert-message", {
            autoAlpha: 0,
            scale: 0.25,
            duration: 1.25,
            ease: "elastic.in(1.4,0.8)",
        ...
        if (keyword.index != 0) {
          ...
          if (count != 0) {
            var messageText = document.createTextNode(
              message.slice(lastKeyword + 2, keyword.index)
            );
          } else {
            var messageText = document.createTextNode(
              message.slice(lastKeyword, keyword.index)
            );
          }
      ...
      if (lastKeyword < getMessage(activeAlert.value).length) {
        ...
        messageText = document.createTextNode(
          getMessage(activeAlert.value).slice(lastKeyword + 2, message.length)
        );
        messageElement.appendChild(messageText);
        userMessage.appendChild(messageElement);
      }
      if (typeof activateAlert.value.attachedMsg !== "undefined") {
        ...
        let messageContent = $(
          `<div class="user-msg">${activateAlert.value.attachedMsg}</div>`
        );
