...
type jChat = typeof Chat
interface ChatInstance extends jChat {}
...
var Chat = {
  info: {
    ...
    cheers: {},
    lines: [],
    blockedUsers: nodecg.Replicant("blockedUsers"),
    bots: ["streamelements", "streamlabs", "nightbot", "moobot", "fossabot"],
  },

  loadEmotes: function (channelID: string) {
    Chat.info.emotes = {}
    ...
    TwitchAPI("https://api.twitch.tv/v5/users?login=" + Chat.info.channel).done(function (res) {
      ...
      TwitchAPI("https://badges.twitch.tv/v1/badges/global/display").done(function (global) {
        ...
        TwitchAPI(
          ...
          $.getJSON("https://api.frankerfacez.com/v1/_room/id/" + encodeURIComponent(Chat.info.channelID)).done(
            function (res) {
              if (res.room.moderator_badge) {
                Chat.info.badges["moderator:1"] =
                  "https://cdn.frankerfacez.com/room-badge/mod/" + Chat.info.channel + "/4/rounded"
              }
    ...
    if (info) {
      ...
      messageNodes.each(function (i) {
        if (
          i != 0 &&
          $(this).data("zw") &&
          ($(messageNodes[i - 1]).hasClass("emote") || $(messageNodes[i - 1]).hasClass("emoji")) &&
